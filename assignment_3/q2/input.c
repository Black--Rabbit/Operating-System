...
/*
 * Main file which will take input and call the search function
 */
...
#include "header.h"
	...
	int main(int argc, char *argv[])
	{
		...
		fclose(fp);
		if(!fork())
			search("/home/user/evaluator/", file);
		wait(NULL);
		delete("flag.txt");
		...
		while((c = fgetc(fp))!= EOF)
		{
			...
			fscanf(fp, "%[^\n]s", temp);
			fgetc(fp);
			if(!strncmp(id, temp, 12))	// find the id
			{
				int i = 13;
				int x = 0;
		...
		fclose(wfp);
		fclose(fp);
		move_file("new_marks.txt", loc);	// replace the file
//		delete("new_marks.txt");
//		delete("path.txt");
